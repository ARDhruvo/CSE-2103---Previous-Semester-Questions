...
void nodeLevel(node *search, int level, int val)
{
    if(search != NULL)
    {
        nodeLevel(search ->left, level, val);
        level++;
        if(search ->data == val)
        {
            cout << val << " level is " << level;
        }
        nodeLevel(search ->right, level, val);
    }
}
...
//     cout << "Enter the value to check level of: ";
//     int y;
//     cin >> y;
//     nodeLevel(root, 0, y);
// }
